...
void Assets::loadSpriteSheet(const std::string& path)
{
    std::ifstream file(path);
    if (!file)
    {
        std::cerr << "Failed to open sprite sheet descriptor: " << path << "\n";
        return;
    }

    std::string token;
    SpriteSheetDescriptor currentDesc;
    bool hasActiveDescriptor = false;

    while (file >> token)
    {
        if (token == "Texture")
        {
            if (hasActiveDescriptor)
            {
                buildAnimationsFromDescriptor(currentDesc);
                currentDesc = SpriteSheetDescriptor();
            }
            ...
            currentDesc.textureName = name;
            hasActiveDescriptor = true;
        }
        else if (token == "RowConfig")
        {
            RowConfigData row;
            file >> row.action
                 >> row.advanceRow
            ...
            row.frames.reserve(row.frameCount);
            row.offsets.reserve(row.frameCount);

            for (size_t i = 0; i < row.frameCount; ++i)
            {
                size_t frameVal, offsetVal;
                file >> frameVal >> offsetVal;
                row.frames.push_back(frameVal);
                row.offsets.push_back(offsetVal);
            }

            currentDesc.rowConfigs.push_back(std::move(row));
        }
        else
        {
            std::cerr << "Unknown SpriteSheet token: " << token << '\n';
        }
    }
