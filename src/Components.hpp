...
// Core transform for all entities
...
// Physics + Locomotion state (reusable for player, NPCs, creatures, etc.)
...
// Bobbing behavior (player camera + potentially other floating things)
class CBob : public Component
{
public:
...
// Player identity / player-only data
class CPlayer : public Component
{
public:
    // Add player-specific things here later (health, stamina, inventory flags, etc.)
    CPlayer() = default;
};
