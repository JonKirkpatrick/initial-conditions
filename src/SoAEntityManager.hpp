...
class ComponentArray {
    std::array<T, MAX_ENTITIES> m_values;
    std::array<EntityHandle, MAX_ENTITIES> m_denseEntities;
    std::array<uint32_t, MAX_ENTITIES> m_denseSlotByEntity; // entity index -> dense slot or invalid
    uint32_t m_liveCount = 0;

public:
    ComponentArray() {
        m_denseSlotByEntity.fill(std::numeric_limits<uint32_t>::max());
    }
