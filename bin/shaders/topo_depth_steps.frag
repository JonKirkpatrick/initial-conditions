...
vec2 encodeU16(float normalizedValue) {
    float packed = floor(clamp(normalizedValue, 0.0, 1.0) * 65535.0 + 0.5);
    return vec2(floor(packed / 256.0) / 255.0, mod(packed, 256.0) / 255.0);
}
...
void main() {
    ...
    bool forceSky = rayDir.y > 0.0 && cameraPos.y >= topdownHeightMax;
    ...
    if (isHit) {
        normalizedDepth = clamp((t - nearPlane) / (farPlane - nearPlane), 0.0, 1.0);
    }

    float stepNorm = clamp((float(stepCount) / u_stepCountNormalizationMax) * u_stepContributionScale, 0.0, 1.0);
    vec2 depth16 = encodeU16(normalizedDepth);
    gl_FragColor = vec4(stepNorm, depth16.x, depth16.y, isHit ? 1.0 : 0.0);
}